...
inline ColorRgb average_region(uint32_t w,uint32_t h,const uint32_t*p,uint32_t stride,
                               uint32_t x0,uint32_t y0,uint32_t x1,uint32_t y1){
    x0=std::min(x0,w); x1=std::min(x1,w);
    y0=std::min(y0,h); y1=std::min(y1,h);
    if(x0>=x1||y0>=y1||stride==0)return{};

    // Ambilight does not need every pixel. Cap the sampling grid so 4K/high-FPS
    // games do not spend most of a CPU core averaging large edge rectangles.
    const uint32_t rw=x1-x0, rh=y1-y0;
    const uint32_t sx=std::max<uint32_t>(1,(rw+63)/64);
    const uint32_t sy=std::max<uint32_t>(1,(rh+31)/32);
    uint64_t sr=0,sg=0,sb=0,n=0;
    for(uint32_t y=y0;y<y1;y+=sy){
        const uint32_t*row=p+(size_t)y*stride;
        for(uint32_t x=x0;x<x1;x+=sx){
            uint32_t q=row[x];
            sb+=(q>>0)&255; sg+=(q>>8)&255; sr+=(q>>16)&255; ++n;
        }
    }
    if(!n)return{};
    return enhance_color({(uint8_t)(sr/n),(uint8_t)(sg/n),(uint8_t)(sb/n)});
}
inline void compute_colors(uint32_t w,uint32_t h,const uint32_t*p,uint32_t stride,LedFrame&out){
    if(!p||!w||!h||!stride){out.fill({});return;}
    float pct=clamp01(cfg_store().capture_depth_percent/100.f);
    uint32_t depth=std::max<uint32_t>(1,(uint32_t)(pct*std::min(w,h)));
    std::array<ColorRgb,kRightCount>right{};
    std::array<ColorRgb,kTopCount>top{};
    std::array<ColorRgb,kLeftCount>left{};
    for(size_t i=0;i<kRightCount;i++){
        uint32_t y0=(uint64_t)i*h/kRightCount,y1=(uint64_t)(i+1)*h/kRightCount;
        right[i]=average_region(w,h,p,stride,w>depth?w-depth:0,y0,w,y1);
    }
    for(size_t i=0;i<kTopCount;i++){
        uint32_t x0=(uint64_t)i*w/kTopCount,x1=(uint64_t)(i+1)*w/kTopCount;
        top[i]=average_region(w,h,p,stride,x0,0,x1,std::min(depth,h));
    }
    for(size_t i=0;i<kLeftCount;i++){
        uint32_t y0=(uint64_t)i*h/kLeftCount,y1=(uint64_t)(i+1)*h/kLeftCount;
        left[i]=average_region(w,h,p,stride,0,y0,std::min(depth,w),y1);
    }
    for(size_t i=0;i<kRightCount;i++)out[i]=right[i];
    for(size_t i=0;i<kTopCount;i++)out[kRightCount+i]=top[i];
    for(size_t i=0;i<kLeftCount;i++)out[kRightCount+kTopCount+i]=left[i];
}
